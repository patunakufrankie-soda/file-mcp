...
def _collect_table_rows(lines: list[str], start_index: int) -> tuple[list[list[str]], int] | None:
    ...
    while index < len(lines):
        ...
        if "|" not in candidate:
            break
        if _HEADING_RE.match(candidate) or _ORDERED_ITEM_RE.match(candidate) or _BULLET_ITEM_RE.match(candidate):
            break
...
def parse_markdown_blocks(content: str) -> list[MarkdownBlock]:
    ...
    while index < len(lines):
        ...
        if table_rows is not None:
            ...
            blocks.append(MarkdownBlock(kind="table", text="", rows=tuple(tuple(cell for cell in row) for row in rows)))
            index = next_index
            continue
        ...
        if heading_match:
            ...
            blocks.append(MarkdownBlock(kind="heading", text=text.strip(), level=len(hashes)))
            index += 1
            continue
        ...
        if ordered_match:
            ...
            blocks.append(MarkdownBlock(kind="ordered_item", text=text.strip(), level=int(number)))
            index += 1
            continue
        ...
        if bullet_match:
            flush_paragraph()
            blocks.append(MarkdownBlock(kind="bullet_item", text=bullet_match.group(1).strip()))
            index += 1
            continue
        ...
        if image_match:
            ...
            blocks.append(MarkdownBlock(kind="image", text=alt_text.strip(), image_src=image_src.strip()))
            index += 1
            continue
...
def parse_markdown_inlines(text: str) -> list[MarkdownInlineSpan]:
    ...
    def flush_buffer() -> None:
        if buffer:
            spans.append(MarkdownInlineSpan(text="".join(buffer), styles=current_styles()))
            buffer.clear()
...
def render_markdown_inlines_as_html(text: str) -> str:
    ...
    for span in parse_markdown_inlines(text):
        ...
        if not span.styles:
            fragments.append(escaped_text)
            continue

        wrapped = escaped_text
        if "code" in span.styles:
            fragments.append(f"<code>{wrapped}</code>")
            continue
...
def render_markdown_inlines_as_reportlab(text: str) -> str:
    ...
    for span in parse_markdown_inlines(text):
        ...
        wrapped = escaped_text
        if "code" in span.styles:
            fragments.append(f'<font face="Courier">{wrapped}</font>')
            continue
        if "strike" in span.styles:
